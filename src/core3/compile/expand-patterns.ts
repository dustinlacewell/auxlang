...
export function expandPatterns(
	...
	for (const node of nodes) {
		for (const [port, value] of Object.entries(node.inputs)) {
			...
			node.inputs[port] = patsigRef(value.pattern, clockNode, seed, `${node.module}.${port}`);
			expanded = true;
		}
	}
	return expanded;
...
function patsigRef(pattern: unknown, clockNode: GNode | null, seed: number, where: string) {
	if (clockNode === null) {
		throw new Error(
			`pattern input at ${where} needs an ambient clock — call clock(bpm) in the eval first`,
		);
	}
